...
Status: phase-4 implementation complete in source; CI/runtime verification pending. D-01–D-03, D-07, D-09, and D-13 were explicitly approved before implementation.

## Implemented contract details

- Requests use an `input` object matching `CommitmentDefinitionDto`. It carries effective dates, a user-owned account/optional space, exact EUR amount, frequency, and explicit payment schedule.
- A monthly due day is optional; an unknown day creates no invented due payment. Annual installments are explicit month/day/amount entries, up to 24 per revision. A positive annual amount requires a schedule whose amounts sum exactly to the annual total; zero remains a valid configured amount.
- Previews take an explicit planning month. Full-month planning applicability and exact due-date applicability remain separate: a scheduled payment outside the source's active calendar dates is not shown, even when the month has a full planning charge.
- Same-month corrections append immutable revisions. Monthly reads select effective month and then highest revision version. Future revisions and archived-source boundaries determine whether an old destination is still needed.
- Lists provide name, kind, account, month, and archived filters before response pagination. The personal-scale revision projection is evaluated within the authenticated user's candidate dataset; response pages are bounded.
- Input provenance is exact JSON; obligation/installment amounts are normalized BIGINT cents. The calculation version is `commitment-v1`. Every accepted write has a transactional audit event.
- The current categories include financing and planned investment charges without creating financing balances or actual investment operations. Their specialized records remain phase 6 and must link to the existing canonical source rather than duplicating its charge.
...
9. Approved D-13: month/day schedules clamp to the final valid day when that day does not exist, including February 29 in non-leap years. This does not imply moving weekend/holiday dates.
...
An annual EUR 120.00 obligation first included in October contributes EUR 10.00 in each remaining active month under approved D-02. Do not claim those EUR 30.00 fund a EUR 120.00 November bill. Show the payment requirement and the absence of verified reserve information. Additional catch-up or opening-funding behavior requires separately specified inputs and rules.
...
The owner approved the phase-4 provision, validity, installment, and due-day policies. The full-cycle conservation guarantee does not imply that a partial active year accumulates the full annual amount. Actual reserve balances and payment execution remain outside this planning configuration.
