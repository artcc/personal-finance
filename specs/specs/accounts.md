...
Status: phase-4 destinations implemented; CI/runtime verification pending. Monthly allocation behavior remains phase 5 and depends on D-06/D-11.

## Implemented contract details

- `/accounts` supports page/pageSize (default 25, maximum 100), name/institution/reference search, selected month, and inclusion of archived accounts.
- `/accounts/{id}/spaces` lists spaces with bounded pagination. An account supports up to 100 spaces; creating/editing/archiving a space increments the parent account version so archive reviews cannot silently omit concurrent child changes.
- Names, institution labels, and display references are limited to 120 characters. EUR is the initial supported currency. No observed bank balance is entered or inferred.
- Space relationships use composite database foreign keys including account and user identifiers. Responses contain explicit account/space DTOs rather than raw Prisma objects.
- Both account and space archive previews exist. They return affected spaces, a current version, up to 20 future source references, and the full reference count. The final archive request includes the reviewed space IDs and rechecks references in a transaction.
- Financial writes serialize on the authenticated user's row. Ownership, versions, future references, child archiving, and audit events are checked/applied within that transaction; no database connection is acquired outside the transaction while holding its lock.
- Future income/commitment revisions can reassign a destination before archiving. Superseded revisions and one-month entries outside the archive window do not falsely block archive.
- Archived destination configuration is read-only. Historical data remains present; the interface does not implement unarchiving in this phase.
