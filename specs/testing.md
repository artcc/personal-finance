...
The frontend session-cache regression tests use real QueryObserver subscriptions to verify that already mounted guards see login/logout transitions and that private cached data is removed. They run in `pnpm test:unit` after the existing CI build generates the API client. E2E logout assertions first require the server's 204 response and cookie removal, then check redirects in the active and secondary tabs; do not replace these checks with longer navigation timeouts.

Phase-4 coverage adds exact tax/hourly calculations, annual cent conservation, inclusive dates, due-day clamping, installment totals, locale input/formatting, immutable revisions, user/relationship isolation, one-month income, stale concurrent edits, and atomic archive/reference checks. A browser journey configures an account, salary, and annual commitment and verifies that changed inputs invalidate a previously approved preview.
...
The owner subsequently reported phase-5 CI green. Phase-6 tests now cover exact unit balances, unknown opening capital, sales without enough units, same-day correction order, invalid void rollback, dated debt/valuation records, duplicate/cross-user source links, protected linked-source categories, and the separation between actual movements and monthly plan charges. A browser journey creates a financing and investment, records debt and unit movements, and enters a manual valuation. These new tests have not been run locally; current CI evidence is pending.

The owner subsequently reported phase-6 CI green. The current JSON-portability tests cover exact file version/model validation, unknown fields, broken references, identity remapping, exclusion of authentication data, closed-plan amount preservation, rejection of nonempty destinations, and a database-failure probe that verifies rollback of earlier imported collections. A browser test downloads the JSON and imports it into a different empty authenticated workspace. These new tests remain CI-only and unexecuted locally.
